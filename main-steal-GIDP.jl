        ...
        if outs  == 2
            value = f(State(b, j - 1, outs-1, base1, base2, base3)) * OutGet(b_next) +
                HomeRun(b_next) * sum(f(State(b, j - 1, outs, b1, b2, b3)) for b1 in [0, 1], b2 in [0, 1], b3 in [0, 1]) + 
		            .25 * sum(f(State(b, j - 1, 0, b1, b2, b3)) for (b1,b2,b3) in [(1,0,0), (1,1,0)]) + 
...
function Single(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 5]
    end
...
function Double(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 6]
    end
...
function Triple(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 7]
    end
...
function HomeRun(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 8]
    end
...
function Walk(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 4]
    end
...
function OutGet(i::Int)::Float64
    ...
    global flag
    if flag == "uniform"
        return playersData[penultimate_index, 2] + playersData[penultimate_index, 3]
    end
...
function DoubleOut(i::Int)::Float64
    ...
    global flag 
    if flag == "uniform"
        return playersData[penultimate_index, 11]
    end
...
for i in 1:1 #nrow(ludf)
    ...
    if haskey(seenLineups, lineup_str)
        ...
        if length(lineup) == 9
          ...
          populateMemo()
          eearly = expectedRunsNormed(MAX_R, NUM_INNINGS - 1)
          efull = expectedRunsNormed(MAX_R, NUM_INNINGS)
...
CSV.write("v2_probs-steals-IBB-GIDP-uniform.csv", ludf,append=true)
